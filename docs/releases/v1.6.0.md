...
**NOTE:** If READY status is `FALSE` one of the possible problem can be `too many certificates already issued for xip.io`

```bash
kubectl describe order -n kube-system nginx-test-certificate-66752116-48799971 | tail -n 5
Status:
  Failure Time:  2020-06-17T09:12:39Z
  Reason:        Failed to create Order: 429 urn:ietf:params:acme:error:rateLimited: Error creating new order :: too many certificates already issued for: xip.io: see https://letsencrypt.org/docs/rate-limits/
  State:         errored
Events:          <none>
```
In this scenario used `nip.io` instead of `xip.io`
